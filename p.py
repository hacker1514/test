...
def on_focus_out(event):
    ...
    for widget in w.winfo_children():
        widget.destroy()
    result_label = Label(w, text=f"Your Score: {correct_count} / {len(questions)}",
                         font=("Arial", 40, "bold"), fg="white", bg="#1c1c1c")
...
def show_question(index):
    ...
    for opt in opts:
        rb = Radiobutton(q_area, text=opt, variable=sv, value=opt, font=("Arial", 14), bg="#1c1c1c", fg="white",
                         selectcolor="#1c1c1c", activebackground="#1c1c1c", anchor="w", justify=LEFT)
